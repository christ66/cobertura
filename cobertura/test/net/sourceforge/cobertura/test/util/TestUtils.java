...
public class TestUtils {
	...
	public static AntBuilder getCoberturaAntBuilder(File coberturaClassdir) {
		...
		project.addTaskDefinition("groovyc", org.codehaus.groovy.ant.Groovyc.class);
	...
	public  static Node getXMLReportDOM(String xmlReport) throws ParserConfigurationException, SAXException, IOException {
	...
	public static void instrumentClasses(AntBuilder ant, File srcDir, File datafile, File instrumentDir, Map arguments) {
		...
		instrumentTask.setThreadsafeRigorous(
				arguments != null && arguments.containsKey("threadsafeRigorous") ? (Boolean)arguments.get("threadsafeRigorous") : false);
		instrumentTask.setIgnoreTrivial(
				arguments != null && arguments.containsKey("ignoretrivial") ? (Boolean)arguments.get("ignoretrivial") : false);
		
		if (arguments != null && arguments.containsKey("ignoreAnnotationNames")) {
			List<String> ignoreAnnotations = (List<String>) arguments.get("ignoreAnnotationNames");
			for (String annotation : ignoreAnnotations) {
				instrumentTask.createIgnoreMethodAnnotation().setAnnotationName(annotation);
			}
		}
		
		if(arguments != null && arguments.containsKey("excludeClassesRegexList")) {
			instrumentTask.createExcludeClasses().setRegex((String)arguments.get("excludeClassesRegexList"));
		}
	...
	public static List<Node> getLineCounts(Node dom, String className, String methodName, String signature) {
											...
											for (Iterator<Node>methodsIterator = classNode.iterator(); methodsIterator.hasNext();) {
												...
												if ("methods".equals(methodsNode.name())) {
													for(Iterator<Node>methodIterator = methodsNode.iterator(); methodIterator.hasNext();) {
														...
														if ("method".equals(methodNode.name())) {
															if (methodName.equals(methodNode.attribute("name"))) {
																if (signature != null){returnList.clear();} // Keep clearing if signature isn't null
																for(Iterator<Node>linesIterator = methodNode.iterator(); linesIterator.hasNext();) {
																	Node linesNode = linesIterator.next();
																	if ("lines".equals(linesNode.name())) {
																		for (Iterator<Node>lineIterator = linesNode.iterator(); lineIterator.hasNext();) {
																			Node lineNode = lineIterator.next();
																			if ("line".equals(lineNode.name())) {
																				returnList.add(lineNode);
																			}
																		}
																	}
																}
																if (signature != null) {
																	if(signature.equals(methodNode.attribute("signature"))) {
																		return returnList;
																	} else {
																		returnList.clear();
																	}
	...
	public static void instrumentClasses(AntBuilder ant, File srcDir, File datafile, File instrumentDir) {
		instrumentClasses(ant, srcDir, datafile, instrumentDir, null);
	}

	public static void checkFrameSummaryHtmlFile(File frameSummaryFile) throws Exception {
		XmlParser parser = new XmlParser();
		...
		for (Iterator<Node> tableIterator = list.iterator(); tableIterator.hasNext();) {
			Node tableNode = tableIterator.next();
			if ("table".equals(tableNode.name())) {
				if("report".equals(tableNode.attribute("class"))) {
					for(Iterator<Node>tbodyIterator = tableNode.iterator(); tbodyIterator.hasNext();) {
						Node tbodyNode = tbodyIterator.next();
						if ("tbody".equals(tbodyNode.name())) {
							for(Iterator<Node>trIterator = tbodyNode.iterator(); trIterator.hasNext();) {
								...
								if ("tr".equals(trNode.name())) {
									...
									int tdClassFirstOccurance = value.indexOf(tdClassString);
									
									String tdClassStart = value.substring(tdClassFirstOccurance);
									int tdClassElement = Integer.valueOf(tdClassStart.substring(tdClassString.length(),
																							    tdClassStart.indexOf("]],")));
									if (trNode.value().toString().contains("All Packages")) {
		...
		assertFalse("All classes should not be 0.", 0 == expectedTotalNumberOfClasses);
	...
	public static List<Node> getLineCounts(Node dom, String className, String methodName) {
	...
	public static void waitForLiveServer(String webContainerHostname , int webContainerPort, int timeoutMin) {
		...
		while((!portOpened) && (System.currentTimeMillis() < endTime)) {
			...
			if (portOpened) {
				System.out.println("Web server has opened the port in " + (System.currentTimeMillis() - beginTime)/1000.0/60.0 + " min.");
			} else {
				try {
	...
	public static boolean isMethodHit(Node dom, String className,String methodName) {
		List<Node> methods = TestUtils.getLineCounts(dom, className, methodName);
